...
class Store {
  ...
  _getBalanceOfs = async (web3, multicall, baseAssets, account) => {
    try {
      console.log("inside _getBalanceOfs")
      const balanceOfCalls = baseAssets.map((asset) => {
        if(asset.address === 'BSC') {
  ...
  createPairStake = async (payload) => {
    try {
      ...
      if(token0.address === 'BSC') {
        toki0 = CONTRACTS.WFTM_ADDRESS
      }
      if(token1.address === 'BSC') {
  ...
  quoteSwap = async (payload) => {
    try {
      ...
      if(fromAsset.address === 'BSC') {
        addy0 = CONTRACTS.WFTM_ADDRESS
      }
      if(toAsset.address === 'BSC') {
        addy1 = CONTRACTS.WFTM_ADDRESS
      }
  ...
  _callContractWait = (web3, contract, method, params, account, gasPrice, dispatchEvent, dispatchContent, uuid, callback, paddGasCost, sendValue ) => 
  {
    console.log("method----",method)
    console.log("params-----",params)
    console.log("contract",contract)
    console.log("gasPrice",gasPrice)
    console.log("paddGasCost",paddGasCost)
    ...
    const gasCost = contract.methods[method](...params)
      ...
      .then((gasAmount,hh) => {
        console.log("gasAmount",gasAmount,hh)
        const context = this
        ...
        console.log("sendGasPrice",sendGasPrice)
        // if (paddGasCost) {
